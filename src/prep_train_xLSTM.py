# ===========================================================
# combined_prep_and_tpe_train_xlstmts.py
# Full pipeline: preprocessing + TPE hyperparameter tuning + verdict
# ===========================================================
...
# ----------------------
INPUT_SIZE = 1          # we feed only TARGET_COL to the model (univariate)
EMBED_DIM = 64
OUTPUT_SIZE = 1
WEIGHT_DECAY = 0.0
MAX_EPOCHS = 200
...
# xLSTM block configs (per your spec)
MLSTM_CONV_K = 4
MLSTM_PROJ_SIZE = 2
MLSTM_HEADS = 2
SLSTM_CONV_K = 2
...
# ----------------------
# TPE search space (exact choices requested)
# ----------------------
SEQ_LEN_CHOICES = [100, 150, 200]
LR_CHOICES = [1e-4, 5e-4, 1e-4]   # includes the duplicate 0.0001 as requested
...
def scale_with_darts(train_ts: TimeSeries, other_ts_list: List[TimeSeries]) -> Tuple[Scaler, List[TimeSeries]]:
    scaler = Scaler()  # MinMaxScaler(0,1) by default
    scaler.fit(train_ts)
...
class xLSTM_TS(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.head(x[:, -1, :])  # next-step prediction
...
def load_scaled_split(path: str, target_col: str) -> pd.Series:
    ...
    assert target_col in df.columns, f"Target '{target_col}' not in {path}"
    return df[target_col].astype("float32").dropna()

def make_arrays(seq_len: int) -> Dict[str, np.ndarray]:
    train_s = load_scaled_split("scaled_train_businessB_denoised.csv", TARGET_COL)
    val_s   = load_scaled_split("scaled_val_businessB_denoised.csv", TARGET_COL)
    test_s  = load_scaled_split("scaled_test_businessB_denoised.csv", TARGET_COL)
    Xtr, ytr = build_windows_target_only(train_s, window=seq_len, horizon=1)
    Xva, yva = build_windows_target_only(val_s,   window=seq_len, horizon=1)
    Xte, yte = build_windows_target_only(test_s,  window=seq_len, horizon=1)
...
# ------------------------------------------------------------
# Preprocess end-to-end (keeps your full functionality)
# ------------------------------------------------------------
def preprocess_and_save() -> None:
    ...
    # Save scaled splits for downstream windowing
    ...
    log("Saved scaled CSV splits.")

# ------------------------------------------------------------
# Optuna TPE search
# ------------------------------------------------------------
def objective(trial: optuna.Trial) -> float:
    # Sample hyperparameters with TPE
    ...
    # Build arrays & loaders for this sequence length
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # Pruning (optional): enable if you like
    ...
    # Persist best weights into user attrs so we can reload after study
...
def train_eval_best(study: optuna.Study) -> Dict[str, Any]:
    ...
    # Rebuild data with best seq_len
    ...
    # Compute a naïve "persistence" baseline on test: predict last value in window
    Xte, yte = arrs["Xte"], arrs["yte"]
    if Xte.shape[0] > 0:
        naive_pred = Xte[:, -1, 0]  # last observed value
        baseline_mse = float(np.mean((naive_pred - yte) ** 2))
    else:
    ...
    # Verdict logic (scaled space; compare to baseline)
...
def verdict_from_metrics(test_mse: float, baseline_mse: float) -> str:
    # Use improvement vs baseline + absolute RMSE thresholds in scaled space
    rmse = float(np.sqrt(test_mse)) if math.isfinite(test_mse) else float("inf")
    if math.isfinite(baseline_mse):
        imp = (baseline_mse - test_mse) / baseline_mse if baseline_mse > 0 else 0.0  # relative improvement
    else:
        imp = 0.0

    # Heuristic thresholds (scaled to [0,1]): lower RMSE and bigger improvement is better
    # Excellent: >=25% better than baseline AND RMSE <= 0.03
    # Good:      >=10% better than baseline AND RMSE <= 0.05
    # Fair:      any improvement over baseline OR RMSE <= 0.07
    # Poor:      otherwise
    if imp >= 0.25 and rmse <= 0.03:
        return "EXCELLENT: large improvement over naïve baseline with very low error."
    elif imp >= 0.10 and rmse <= 0.05:
        return "GOOD: clear improvement over baseline with low error."
    elif imp > 0.0 or rmse <= 0.07:
        return "FAIR: modest improvement or acceptable error; may need more tuning/data."
    else:
        return "POOR: does not beat baseline meaningfully; consider revising features/model/tuning."
...
def main() -> None:
    set_seed(SEED)
    # 1) Preprocess (full functionality preserved)
    preprocess_and_save()

    # 2) TPE Hyperparameter optimization
    sampler = TPESampler(seed=SEED)
    study = optuna.create_study(direction="minimize", sampler=sampler)
    print("Starting TPE search over {seq_len, lr, batch_size} ...")
    study.optimize(objective, n_trials=20, show_progress_bar=True)  # adjust n_trials as you like

    # 3) Evaluate & save best model, print verdict
