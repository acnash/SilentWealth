...
# metrics, and graphs per run. Replaces Optuna TPE tuning
# with a reproducible Grid Search over the same ranges.
# ===========================================================
...
# === CHANGED: Predict 3 days ahead (horizon = 3) ===
...
# Search spaces (identical ranges/choices to Optuna)
...
NUM_LAYERS_CHOICES = [2, 4, 6]
LR_RANGE = (1e-6, 3e-3)  # min/max unchanged; grid will sample log-uniformly inside this range
LR_GRID_POINTS = 3
...
def predict_and_report(model: nn.Module, loader: DataLoader, X: np.ndarray, y: np.ndarray, tickers: np.ndarray, out_prefix: str) -> Dict[str, Any]:
    ...
    for tkr in unique_tkrs:
        ...
        m_row = {"Ticker": tkr}; m_row.update(m)
        rows.append(m_row)
...
# ------------------------------------------------------------
# Grid Search utilities (replacing Optuna)
# ------------------------------------------------------------
def lr_log_grid(n_points: int) -> List[float]:
    """
    Return a log-uniform grid of learning rates INCLUDING the exact endpoints,
    strictly within the original Optuna range [LR_RANGE[0], LR_RANGE[1]].
    """
    n_points = max(2, int(n_points))
    return np.geomspace(LR_RANGE[0], LR_RANGE[1], num=n_points, endpoint=True).tolist()

def train_eval_config(seq_len: int, lr: float, batch_size: int, embed_dim: int, num_layers: int, arch_style: str) -> Dict[str, Any]:
    batch_sz = batch_size  # keep local name for existing code
    """
    Train with early stopping on a given hyperparameter config.
    Returns a dict containing best val loss, learning curve, and model state.
    """
    # heads divisibility constraint (matches original)
...
def evaluate_best_and_save(best: Dict[str, Any]) -> Dict[str, Any]:
    """
    Using the best config/state dict, regenerate arrays/loaders,
    run full prediction reports for val/test, save plots & artifacts.
    """
    ...
    best_state = best.get("best_state", None)
    ...
    y_true_val_abs = val_report["y_true_abs"];  y_pred_val_abs = val_report["y_pred_abs"]
    y_true_test_abs = test_report["y_true_abs"]; y_pred_test_abs = test_report["y_pred_abs"]
    plot_parity(y_true_val_abs,  y_pred_val_abs,  os.path.join(CURRENT["artifact_dir"], "parity_val.png"),  f"{CURRENT['run_id']} — Parity (Validation)")
    plot_parity(y_true_test_abs, y_pred_test_abs, os.path.join(CURRENT["artifact_dir"], "parity_test.png"), f"{CURRENT['run_id']} — Parity (Test)")
    plot_residual_hist(y_pred_val_abs - y_true_val_abs,  os.path.join(CURRENT["artifact_dir"], "residuals_val.png"),  f"{CURRENT['run_id']} — Residuals (Validation)")
    plot_residual_hist(y_pred_test_abs - y_true_test_abs, os.path.join(CURRENT["artifact_dir"], "residuals_test.png"), f"{CURRENT['run_id']} — Residuals (Test)")
    ...
    with open(hparam_path, "w") as f:
        json.dump({
            ...
            "best_params": hp,
            "best_val_mse_during_tuning": best.get("best_val", float("nan")),
            "val_mse": val_mse,
    ...
    print(f"\n=== {CURRENT['run_id']} — GRID SEARCH BEST SUMMARY ===")
    print(f"Best params: {hp}")
    print(f"Best val MSE during search: {best.get('best_val', float('nan')):.6f}")
...
def grid_search_configs(lr_points: int, max_configs: Optional[int]=None) -> List[Dict[str, Any]]:
    """
    Build the full hyperparameter grid (cartesian product) using the EXACT SAME
    ranges/choices as the original Optuna study. Learning rate is sampled
    log-uniformly within [LR_RANGE[0], LR_RANGE[1]].
    Optionally cap total combinations with max_configs (deterministic truncation).
    """
    lrs = lr_log_grid(lr_points)
    grid = []
    for seq_len in SEQ_LEN_CHOICES:
        for batch_size in BATCH_CHOICES:
            for embed_dim in EMBED_DIM_CHOICES:
                for num_layers in NUM_LAYERS_CHOICES:
                    for arch_style in ARCH_STYLE_CHOICES:
                        for lr in lrs:
                            grid.append({
                                "seq_len": seq_len,
                                "lr": float(lr),
    ...
    if max_configs is not None and max_configs > 0 and len(grid) > max_configs:
        # deterministic truncation preserves ordering
        grid = grid[:max_configs]
    return grid

def run_grid_search(lr_points: int, max_configs: Optional[int]=None) -> Dict[str, Any]:
    """
    Execute grid search; write a CSV of all tried configs and their best val loss.
    Returns the best config dict enriched with curve and state.
    """
    results = []
    best_entry = None
    grid = grid_search_configs(lr_points, max_configs)

    print(f"Grid size: {len(grid)} combinations (lr_points={lr_points}{', capped' if (max_configs and len(grid)>=max_configs) else ''})")
    for i, cfg in enumerate(grid, 1):
        print(f"[{CURRENT['run_id']}] Grid {i}/{len(grid)} — {cfg}")
        out = train_eval_config(**cfg)
        if out.get("skipped", False):
            results.append({**cfg, "best_val": float("nan"), "status": "skipped"})
            continue
        results.append({**cfg, "best_val": out["best_val"], "status": "ok"})
        if (best_entry is None) or (out["best_val"] < best_entry["best_val"]):
            best_entry = out

    # Save grid results
    res_df = pd.DataFrame(results)
    res_csv = os.path.join(CURRENT["artifact_dir"], "grid_search_results.csv")
...
# Per-file runner
# ------------------------------------------------------------
def run_pipeline_for_file(input_path: str, lr_grid_points: int, max_configs: Optional[int]=None) -> None:
    """
    Run the full pipeline (preprocess -> Grid Search -> evaluate best) for a single input file.
    ...
    print(f"\n[{run_id}] GRID search over {{seq_len, lr(log grid), batch_size, embed_dim, num_layers, arch_style}} …")
    best = run_grid_search(lr_points=LR_GRID_POINTS, max_configs=max_configs)
    if best is None:
        print("No valid configurations were trained (insufficient data across grid).")
        return
    _ = evaluate_best_and_save(best)
...
def main() -> None:
    ...
    # Grid search controls (replacing Optuna):
    p.add_argument("--lr_grid_points", type=int, default=8, help="Number of log-spaced learning rates within [1e-6, 3e-3].")
    p.add_argument("--max_configs", type=int, default=0, help="Optional cap on total grid combinations (0 means no cap).")
    args = p.parse_args()

    max_configs = args.max_configs if args.max_configs and args.max_configs > 0 else None
    ...
    if args.run_mode == "single":
        ...
        if not os.path.exists(args.input_file):
            ...
            return
        try:
            run_pipeline_for_file(args.input_file, lr_grid_points=args.lr_grid_points, max_configs=max_configs)
        ...
        for fp in files:
            try:
                run_pipeline_for_file(fp, lr_grid_points=args.lr_grid_points, max_configs=max_configs)
            except Exception as e:
