...
# Iterate over many training-data combinations (files like xLSTM/GOOGL_PRICES.txt),
# train xLSTM-TS per file, and save models, reports, and graphs per run.
# ===========================================================

import os, math, json, warnings, glob
import numpy as np
import pandas as pd
import pywt
...
CURRENT = {"artifact_dir": None, "train_path": None, "val_path": None, "test_path": None, "run_id": None, "input_path": None}
...
def predict_on_loader(model: nn.Module, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for xb, yb in loader:
        xb = xb.to(DEVICE); yp = model(xb).detach().cpu().numpy().reshape(-1); yb_np = yb.numpy().reshape(-1)
...
def train_one_epoch(model: nn.Module, loader: DataLoader, opt: torch.optim.Optimizer, loss_fn: nn.Module, scaler: amp.GradScaler) -> float:
    ...
    for xb, yb in loader:
        ...
        if CLIP_MAX_NORM and CLIP_MAX_NORM > 0:
            scaler.unscale_(opt); nn.utils.clip_grad_norm_(model.parameters(), CLIP_MAX_NORM)
...
def run_pipeline_for_file(input_path: str) -> None:
    ...
    print(f"\n[{run_id}] TPE search over {{seq_len, lr, batch_size, embed_dim}} …")
    study.optimize(objective, n_trials=20, show_progress_bar=True)
    _ = train_eval_best(study)

# ------------------------------------------------------------
# Main: iterate all *_PRICES.txt in INPUT_DIR
# ------------------------------------------------------------
def main() -> None:
    files = sorted(glob.glob(os.path.join(INPUT_DIR, "*_PRICES.txt")))
    if not files:
        print(f"No files found in {INPUT_DIR} matching *_PRICES.txt"); return
